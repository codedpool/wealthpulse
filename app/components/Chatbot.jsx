...
const Chatbot = ({ selectedFund }) => {
  ...
  const handleSendMessage = async () => {
    ...
    const prompt = selectedFund
      ? `Context: Discussing ${selectedFund.name} (${selectedFund.code}).\nQuestion: ${input}\n\nProvide a clear, concise analysis or advice about this investment, focusing on the specific query. Format the response in 2-5 key points.`
      : `Question: ${input}\n\nProvide general investment guidance or mutual fund advice, formatted in 2-5 key points. Focus on education and clarity.`;
    ...
    <div className="fixed bottom-5 right-5 z-50">
      {!isOpen ? (
        <button
          ...
          className="w-12 h-12 bg-gradient-to-r from-purple-600 to-cyan-500 rounded-full text-white shadow-lg hover:scale-105 transition-transform flex items-center justify-center"
        >
            ...
            <button
              ...
              className={`mt-2 w-full py-2.5 px-4 rounded-lg font-semibold text-white transition-all transform ${
                loading
                  ...
                  : "bg-gradient-to-r from-purple-600 to-cyan-500 hover:from-purple-700 hover:to-cyan-600 hover:scale-[1.02]"
              }`}
            >
              Send Message
            </button>
